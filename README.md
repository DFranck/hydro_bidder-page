...
- Supabase for data storage
  - Data is built with three scheduled cloud functions, found in the `/functions` directory, and written to Supbase files prefixed with `NEXT_PUBLIC_SUPABASE_DATA_NAMESPACE` and two dashes. For example, production's round data is at `../main--raw-hydro-round-data.json`. Make sure you set this env variable before executing any of the functions locally
...
There are three cloud functions that run on different schedules to build different chunks of our backend data:

- scheduled-build-hydro-meta-data
  - Makes a couple queries for "top-level" metadata about the round, lockups, etc.
- scheduled-build-external-data
  - Fetches data from github and other external data sources
- scheduled-build-hydro-round-data-in-background
  - Builds the core data for all bids from all rounds
  - Is a background function because it can take 10s or more to execute
