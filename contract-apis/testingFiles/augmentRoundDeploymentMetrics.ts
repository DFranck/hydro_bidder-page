...
export function augmentRoundDeploymentMetrics(
  ...
  for (const tribute of roundTributes) {
    ...
    proposalsTributes[proposalId]['value_in_usdc'] += amount / Math.pow(10, decimals) * priceUsd;
    proposalsTributes[proposalId]['value_in_atom'] += amount / Math.pow(10, decimals) * priceUsd / roundPrices['ibc/C4CFF46FD6DE35CA4CF4CE031E643C8FDC9BA4B99AE598E9B0ED98FE3A2319F9'].priceUsd;
  }
    ...
    for (const lockup of lockups) {
      ...
      for (const tranche_info of lockup.per_tranche_info) {
        ...
        if (statusVoting.available) {
          ...
          proposalsVotes[statusVoting.voted].voting_power += statusLockup.voting_power;
          proposalsVotes[statusVoting.voted].locked_atom  += statusLockup.locked_atom;
        }
      }
    }
  }
  ...
  const parsedBids = roundBids.map((bid) => {
    ...
    const proposalPoints          = proposalGithubFields ? proposalGithubFields.points             : [];
    const proposalPointProgramUrl = proposalGithubFields ? proposalGithubFields.pointProgramUrl    : '';
    const estimatedPolAPR         = proposalGithubFields ? proposalGithubFields.minMaxTargetPolApr : [];
    const requestAmount           = proposalGithubFields ? proposalGithubFields.requestAmount      : [];

    // Tribute fields
    const proposalTributes        = proposalsTributes[proposalId];
    const tributeUnderlyingAssets = proposalTributes ? Object.entries(proposalTributes).filter(([key, value]) => key !== 'value_in_usdc' && key !== 'value_in_atom') : [];
    const tributeValueInUsdc      = proposalTributes ? proposalTributes.value_in_usdc : 0;
    const tributeValueInAtom      = proposalTributes ? proposalTributes.value_in_atom : 0;
    ...
    const vote_perc               = proposalVotes ? (proposalVotes.voting_power / total_voting_power_per_tranche[bid.tranche_id]) : 0;
    ...
    return {
      ...
      status          : status,
      // Deployment metrics
      apr_tribute     : status === 'Rejected' ? null : tributeValueInAtom / vote_atom / bid.deployment_duration * 12 * 100,
