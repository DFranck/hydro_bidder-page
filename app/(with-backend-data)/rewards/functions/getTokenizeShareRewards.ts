...
import { ClaimableRewardsSummary } from "../types"
...
export async function getClaimableStakingRewardsSummary(
  ...
  try {
    ...
    const stakingRewards = await client.cosmos.distribution.v1beta1.tokenizeShareRecordReward(rewardsReq)
    const atomRewards = stakingRewards.total.find(entry => entry.denom === UATOM_DENOM)
