...
import { useMarketplaceFilters } from "../../marketplace/context/MarketplaceFiltersContext";
...
export function ListingSuccessModal({
 ...
 const {toggleStatus,filters}=useMarketplaceFilters()
  const action = "unlist";
  const handleChange = () => {};
  const router = useRouter();
  return (
    <ModalWindow isOpen={open} onClose={onClose} className="max-w-[98%]"  propsForBackdrop={{
    className: 'bg-black/30 backdrop-blur-sm',
  }}>
  
       <div className="rounded-xl border-2 border-palette-green bg-black p-0">
              ...
              <div className="flex gap-4 justify-end px-[24px] pb-[24px]">
                    <StyledText
                      as="button"
                   ...
                   onClick={() => {
    if (!filters.status.includes("isMine")) toggleStatus("isMine");
    onClose();
    if(!pathname?.includes("marketplace")) router.push(`/lockups/marketplace`);
  }}
                    >
