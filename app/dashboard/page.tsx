...
export default async function Page() {
    ...
    const currentProposals = await Promise.all(tranches.map((tranche) => fetchProposals(currentRound, tranche.tranche_id)));
    ...
    if (lastRoundExists) {
        const lastProposals = await Promise.all(tranches.map((tranche) => fetchProposals(lastRound, tranche.tranche_id)));
        ...
        lastProposalTranches = tranches.reduce((acc, tranche, idx) => {
            return acc.set(tranche.tranche_id, lastProposals[idx])
        }, new Map<number, Proposal[]>())
    }

    const currentProposalTranches = tranches.reduce((acc, tranche, idx) => {
        return acc.set(tranche.tranche_id, currentProposals[idx])
    }, new Map<number, Proposal[]>())
