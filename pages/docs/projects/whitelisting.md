...
In order to compete in Hydro, the Project will need to fill out a form that can be found [here](#).
...
Liquidity exports use the funds allocated through liquidity buckets, that are held and managed by the Hydro smart contract, with the purpose of being distributed among the Projects competing for liquidity exports. At launch, we expect to have one liquidity bucket of ATOM with the ability to convert it, at the end of the round, to LST of Projects choosing. More liquidity buckets will be added with other AEZ tokens & stablecoins in the future.

Each liquidity bucket will be divided into two tranches that differ in size and requirements:

1. ICS tranche with 30% of the liquidity, exclusive to the chains with an ICS agreement (top-n security, partial-set security) with the Cosmos Hub or Projects deployed on an ICS chain (for example Drop and Mars).
2. General tranche with 70% of the liquidity, where any Project can participate (including ICS chains).

## Hydro Committee Review

The Hydro committee plays a crucial role in the Hydro Project, with its main tasks being monitoring and advisory. At the launch of Hydro v1, the committee will have an executive role in whitelisting Projects, deploying liquidity and managing exports. Future versions of Hydro will incorporate a governance module and integration with the Valence protocol built by Timewave Labs, transforming the Hydro committee into an advisory body with limited decision-making authority.

Its elected members will be responsible for maximizing returns for voters and alerting the community about unwanted behaviors and vulnerabilities.
