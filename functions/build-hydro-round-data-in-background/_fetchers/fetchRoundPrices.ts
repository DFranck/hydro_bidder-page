...
export async function fetchRoundPrices({
  ...
  const numiaPricesEndpoint = getEnvironmentVariable(
    "NEXT_PUBLIC_NUMIA_PRICES_ENDPOINT"
  )
  ...
  const response = await fetch(
    `${numiaPricesEndpoint}?round_id=${roundId}`,
    {
      headers: {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch numia price data: ${response.statusText}`
    )
  }
